...
int s21_check_matrix(matrix_t *A) {  // Проверка на корректность матрицы
  int res = 0;
  ...
  if (s21_check_matrix(A)) {
    ...
    *result =
    ...
    for (int i = 0; i < A->columns; i++) {
      matrix_t tmp = {0};
      int check_minor = s21_minor(A, &tmp, 0, i);
      double tmp_d = 0;
      if (s21_determinant(&tmp, &tmp_d) == 0 && check_minor == 0) {
        *result += A->matrix[0][i] * tmp_d * sign;
        sign = -sign;
